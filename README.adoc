...
**POST /todos** - Création d'un TodoItem
- Code HTTP attendu : `201 Created`
- Les données sont persistées en base de données PostgreSQL

**GET /todos** - Récupération de tous les TodoItems
- Code HTTP attendu : `200 OK`
- La réponse contient une liste JSON avec tous les items persistés
- La règle de gestion RG1 est appliquée : `[LATE!]` est préfixé aux items datés de plus de 24h

**Injection de dépendance**
- Le `TodoItemRepository` est correctement injecté via le constructeur
- Pas de `NullPointerException` lors de l'appel aux endpoints
...
**1. Centralité de la logique métier :** Au cœur de l'architecture se trouve la logique métier, souvent représentée par des entités et des cas d'utilisation. Ces éléments ne dépendent d'aucun cadre extérieur ou technologie spécifique.

**2. Ports :** Les ports sont des interfaces qui définissent les points d'interaction nécessaires entre la logique métier et les agents externes. Il y a généralement des ports d'entrée pour les commandes entrantes (comme les requêtes HTTP) et des ports de sortie pour les interactions sortantes (comme la persistance des données).

**3. Adaptateurs :** Les adaptateurs sont des implémentations concrètes des ports qui adaptent la communication entre la logique métier et les agents externes.
...
**4. Séparation des préoccupations :** L'architecture hexagonale cherche à séparer les préoccupations en isolant la logique métier des détails techniques de l'infrastructure.

Cette approche offre une grande flexibilité : il est possible de remplacer un adaptateur sans modifier la logique métier, facilitant ainsi les tests et l'évolution du système.
...
**Tests implémentés (2-3 tests essentiels) :**

**1. Test du Domaine - TodoItemTest :**
```java
@Test
...
void testFinalContentWithRecentItem() {
    ...
    assertFalse(result.startsWith("[LATE!]"));
    assertEquals("Faire les courses", result);
}
```

**2. Test du Service Applicatif - AddTodoItemServiceTest :**
```java
...
class AddTodoItemServiceTest {
    ...
    void testAddTodoItemCallsPort() {
        TodoItem item = new TodoItem("item-1", Instant.now(), "Test");
...
**3. Test de l'Adaptateur REST - TodoListControllerTest :**
```java
